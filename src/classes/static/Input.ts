...
export default class Input {
	...
	private static isPreventingDefaults: boolean = false
	...
	private static registerDefaultPreventers(): void {
		...
		this.isPreventingDefaults = true
	}
	...
	private static unregisterDefaultPreventers(): void {
		...
		this.isPreventingDefaults = false
	}
	...
	private static keyboardEventHandler(event: KeyboardEvent) {
		...
		const frame = GameLoop.getCurrentFrameId()
	...
	public static getUp(code: Key | Button): boolean {
		...
		return frame === GameLoop.getCurrentFrameId() - 1 && state === false
	}
	...
	public static getDown(code: Key | Button): boolean {
		const { state, frame } = this.getState(code)
		return frame === GameLoop.getCurrentFrameId() - 1 && state === true
	}
	...
	public static getAnyKeyDown(): boolean {
		for (const [key, { frame, state }] of this.states) {
			if (typeof key === 'number') continue
			if (frame == GameLoop.getCurrentFrameId() - 1 && state === true) return true
		}
		return false
	}
	...
	public static getAnyKeyUp(): boolean {
		for (const [key, { frame, state }] of this.states) {
			if (typeof key === 'number') continue
			if (frame == GameLoop.getCurrentFrameId() - 1 && state === false) return true
		}
		return false
	}
	...
	public static get preventDefaults(): boolean {
		return this.isPreventingDefaults
	}

	public static set preventDefaults(value: boolean) {
		if (value === this.isPreventingDefaults) return
